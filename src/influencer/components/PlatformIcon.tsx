...
/**
 * Glyphs de marca reales de cada plataforma — antes se usaban íconos genéricos
 * de lucide-react (una cámara/casita cualquiera para Instagram, una nota musical
 * como aproximación de TikTok porque lucide no lo tiene). Aquí se usan los paths
 * exactos de Bootstrap Icons (MIT, https://icons.getbootstrap.com), que sí cubre
 * las 4 marcas con un mismo estilo de trazo consistente entre sí — se ve como un
 * kit real, no como iconos sueltos de fuentes distintas.
 */
...
/** Ícono redondo de la plataforma, con su glyph de marca real y su color oficial. */
export const PlatformIcon: React.FC<Props> = ({ platform, size = 20, className = "" }) => {
  const color = PLATFORM_COLORS[platform] || "#64748B";
  const path = PLATFORM_ICON_PATHS[platform];
  ...
  return (
    ...
    >
      {path ? (
        <svg width={glyphSize} height={glyphSize} viewBox="0 0 16 16" fill="#fff">
          <path d={path} />
        </svg>
      ) : (
        <span className="text-white font-bold" style={{ fontSize: glyphSize * 0.8 }}>
          {platform?.[0] || "?"}
        </span>
      )}
    </div>
